...
def get_data_loader(data_path, batch_size):
    ...
    data_loader = torch.utils.data.DataLoader(
        torchio.ImagesDataset(subjects, transform=Compose([TrilinearInterpolation((192, 256, 192))])),
...
class TrilinearInterpolation(object):
    ...
    def __call__(self, image):
        ...
        data = data.view(shape[1], shape[2], shape[3]).permute(2, 0, 1)
        shape = data.shape

        corrected_output_size = (self.output_size[2],  self.output_size[0],  self.output_size[1])

        tmp = (1, self.output_size[2],  self.output_size[0],  self.output_size[1])

        ret = F.interpolate(data.view(1, 1, shape[0], shape[1], shape[2]), size=corrected_output_size, mode='trilinear').view(tmp).permute(0, 2, 3, 1)
